 ...
 *
 * @param \BlockstudioVendor\TailwindPHP\Theme $theme
 * @return Variants
 */
function createVariants(\BlockstudioVendor\TailwindPHP\Theme $theme): Variants
{
    ...
    $staticVariant = function (
        ...
        $variants->static(
            ...
            function (&$r) use ($selectors) {
                $r['nodes'] = array_map(
                    fn ($selector) => \BlockstudioVendor\TailwindPHP\Ast\rule($selector, $r['nodes']),
                    $selectors,
                );
            },
            ['compounds' => $compounds],
    ...
    $contentProperties = function () {
        return \BlockstudioVendor\TailwindPHP\Ast\atRoot([
            \BlockstudioVendor\TailwindPHP\Ast\atRule('@property', '--tw-content', [
                \BlockstudioVendor\TailwindPHP\Ast\decl('syntax', '"*"'),
                \BlockstudioVendor\TailwindPHP\Ast\decl('inherits', 'false'),
                \BlockstudioVendor\TailwindPHP\Ast\decl('initial-value', '""'),
            ]),
        ]);
    };
    ...
    $variants->static(
        ...
        function (&$v) use ($contentProperties) {
            $v['nodes'] = [
                \BlockstudioVendor\TailwindPHP\Ast\styleRule('&:before', [
                    $contentProperties(),
                    \BlockstudioVendor\TailwindPHP\Ast\decl('content', 'var(--tw-content)'),
                    ...$v['nodes'],
                ]),
            ];
        },
    ...
    $variants->static(
        ...
        function (&$v) use ($contentProperties) {
            $v['nodes'] = [
                \BlockstudioVendor\TailwindPHP\Ast\styleRule('&:after', [
                    $contentProperties(),
                    \BlockstudioVendor\TailwindPHP\Ast\decl('content', 'var(--tw-content)'),
                    ...$v['nodes'],
                ]),
    ...
    $variants->static('hover', function (&$r) {
        $r['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule('&:hover', [
                \BlockstudioVendor\TailwindPHP\Ast\atRule('@media', '(hover: hover)', $r['nodes']),
            ]),
        ];
    });
    ...
    $variants->functional('aria', function (&$ruleNode, $variant) {
        ...
        if ($value['kind'] === 'arbitrary') {
            $ruleNode['nodes'] = [
                \BlockstudioVendor\TailwindPHP\Ast\styleRule('&[aria-' . quoteAttributeValue($value['value']) . ']', $ruleNode['nodes']),
            ];
        } else {
            $ruleNode['nodes'] = [
                \BlockstudioVendor\TailwindPHP\Ast\styleRule("&[aria-{$value['value']}=\"true\"]", $ruleNode['nodes']),
            ];
        }
    });
    ...
    $variants->functional('data', function (&$ruleNode, $variant) {
        ...
        $ruleNode['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule('&[data-' . quoteAttributeValue($variant['value']['value']) . ']', $ruleNode['nodes']),
        ];
    });
    ...
    $variants->functional('nth', function (&$ruleNode, $variant) use ($normalizeNthValue) {
        ...
        $ruleNode['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule("&:nth-child({$nthValue})", $ruleNode['nodes']),
        ];
    });
    ...
    $variants->functional('nth-last', function (&$ruleNode, $variant) use ($normalizeNthValue) {
        ...
        $nthValue = $normalizeNthValue($value['value']);
        $ruleNode['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule("&:nth-last-child({$nthValue})", $ruleNode['nodes']),
        ];
    });
    ...
    $variants->functional('nth-of-type', function (&$ruleNode, $variant) use ($normalizeNthValue) {
        ...
        $nthValue = $normalizeNthValue($value['value']);
        $ruleNode['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule("&:nth-of-type({$nthValue})", $ruleNode['nodes']),
        ];
    });
    ...
    $variants->functional('nth-last-of-type', function (&$ruleNode, $variant) use ($normalizeNthValue) {
        ...
        $nthValue = $normalizeNthValue($value['value']);
        $ruleNode['nodes'] = [
            \BlockstudioVendor\TailwindPHP\Ast\styleRule("&:nth-last-of-type({$nthValue})", $ruleNode['nodes']),
        ];
    });
