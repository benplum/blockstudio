...
Standalone Markdown pages need frontmatter:
...
you attach data such as `section`, `order`, or `description` and read it from
the PHP API.
...
If a collection has a nested page but an intermediate parent source does not
exist, Blockstudio generates an empty container page for that path. This keeps
the WordPress hierarchy complete.
...
content while avoiding duplicate old-typed posts.
...
Loaders can also return extra local paths to discover. Paths outside the
collection root are allowed only when the
