...
and the original Markdown file path is stored so the page can also be served
as raw Markdown.
...
Markdown-backed pages can expose their source Markdown for tools, agents, and
...
| ------------------------------ | --------- | -------------------------------------------------------------------------------------------------- |
| `collection` / `slug` / `name` | `string`  | Collection slug. `collection` is the clearest form.                                                |
...
Nested relationships require a hierarchical post type. If a collection uses a
non-hierarchical post type, Blockstudio records a discovery error because
WordPress cannot represent the `post_parent` relationship.
...
Pages are synced when you visit the WordPress admin. Blockstudio stores a
fingerprint for each synced page and only updates WordPress when the relevant
source inputs change.

To prevent a page from being overwritten after manual edits, set `"sync": false` in `page.json`.

For collection pages, sync fingerprints include the manifest, loader, page
source, layout file, metadata, and related source paths. When any of those
inputs change, the page is invalidated and synced again.

When a collection source is removed, Blockstudio marks the missing synced page
as stale so orphaned generated pages are pruned instead of lingering forever.
