...
When `assets.reset.enabled` is active, Blockstudio also restores Tailwind's
common display and position utilities inside the editor canvas. This keeps
wrapper-level classes such as `flex`, `grid`, `absolute`, `fixed`, and `sticky`
from being overridden by WordPress editor block wrapper rules, so utility-driven
