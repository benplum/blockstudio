...
Limit the pull to one configured post type:
...
Prune content-set owned database posts that no longer exist in files:

```bash
wp bs content push --prune --yes
```

`--prune` is additive-apply first, cleanup second. The command refuses a
destructive prune unless `--yes` is present.
...
The table reports whether each file-backed entity is missing from the database,
unchanged, would update, conflicted, locked, or invalid.
