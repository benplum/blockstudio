...
With the example above, `<dv-card>` resolves to
`divine-homepage/card`, `<dv-button>` falls back to `bsui/button` when
`divine-homepage/button` is not registered, and
`<dv-onumia-feature-matrix>` resolves to
