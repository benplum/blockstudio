...
Need to register your own type? See [Custom Field Types](/docs/blocks/attributes/custom-field-types).
...
| `default` | `string` | Default value that should be applied when first adding the block. |
| `fallback` | `string` | Fallback value that that will display when field value is empty. |
| `spacingSizes` | `boolean` | If true, uses the Gutenberg spacing size preset control for selecting a single value. |
| `spacingScale` | `array` | Optional custom per-field spacing scale. When set, this is used instead of the global spacing preset control and stores the selected value directly. |
| `units` | `array` |  |
...
| `default` | `string` | Default value that should be applied when first adding the block. |
| `fallback` | `string` | Fallback value that that will display when field value is empty. |
| `spacingSizes` | `boolean` | If true, uses the Gutenberg spacing size interface with linked and per-side editing modes. |
| `spacingScale` | `array` | Optional custom per-field spacing scale. When set, this is used instead of the global spacing preset control and stores selected side values directly. |
| `sides` | `array` | Limits which sides can be edited. Supports top, right, bottom, left and axis aliases horizontal/vertical. |
...
## dimensionSingle Notes

`dimensionSingle` stores one scalar dimensions value (for example `16px`, `2rem`, or `50%`) in the normal attribute flow.
...
Use it like any other scalar field in templates:

```php
<?php echo $a['cardGap']; ?>
```

The field does not auto-apply inline styles. If you want to map its value to markup, use existing extension `set` mappings (for classes, styles, or data attributes).
