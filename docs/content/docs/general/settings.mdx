...
When enabled, Blockstudio caches runtime build payloads and resolved editor
assets. Runtime cache entries are invalidated when watched block files, field
files, asset files, settings, active plugins, WordPress, PHP, or Blockstudio
versions change.
