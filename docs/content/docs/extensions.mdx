...
### dimensionSingle Example

`dimensionSingle` works with `set` like other scalar fields and stores only the value.
...
```json
{
  ...
  "set": [
    {
      "attribute": "class",
      "value": "spacing-{attributes.spacing}"
    }
  ]
}
```

This produces class output such as `spacing-8px`. It does not auto-generate inline styles unless you explicitly map it to `style` in `set`.
