...
description: Bundled UI components, an expanded editor, plugin dependencies, block tag aliases, and a more reliable page parsing pipeline.
date: 2026-05-15
author: Dennis
---
...
<block name="bsui/input" nameAlt="email" type="email" placeholder="Email"></block>
```
...
- `bsui/button`, `bsui/button-group`, `bsui/toggle`, `bsui/toolbar`
- `bsui/input`, `bsui/textarea`, `bsui/select`, `bsui/checkbox`,
  `bsui/radio-group`, `bsui/slider`, `bsui/switch`
...
- `bsui/form`, `bsui/field`, `bsui/field-group`
...
The UI library also has its own documentation and test harness, so component
behavior is not just a collection of templates. It is something the framework
can keep verifying as it grows.
...
the plugin context they need. Block tag aliases make template authoring nicer.
The parser and editor fixes make those compositions more reliable once they
move through WordPress.
